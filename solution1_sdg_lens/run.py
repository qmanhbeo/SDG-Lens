...
def build_run_config(
    ...
    *,
    ...
    return {
        ...
        "dataset_source": str(args.data_dir),
...
```bash
python run.py --load outputs/runs/{run_config['run_id']}/{checkpoint_name} --eval-only
```
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--data_dir", type=Path, default=REPO_ROOT / "SDGi" / "data")
...
def main() -> int:
    ...
    if args.load is not None:
        ...
        args.data_dir = Path(checkpoint.get("data_dir", saved_config.get("data_dir", args.data_dir)))
        ...
        test_frame = load_sdgi_split(args.data_dir, "test", args.language, args.test_samples, args.seed + 1)

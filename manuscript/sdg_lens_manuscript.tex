...
We also add two safeguards to make the work stronger: a small manual rationale check on held-out examples. The results show that SDG Lens is useful as a human-in-the-loop support tool, but it should not replace human judgement in policy or funding decisions.
\end{abstract}
...
While our prototype uses a compact BERT-family model (MiniLM, 6 layers, 22M parameters), the results fit the expectation. TF-IDF outperforms MiniLM on small datasets, but MiniLM shows bigger improvements as data size increases (see Table 2). This is a useful finding because it shows that ``more complex'' is not automatically better for every AI solution.
...
This capability is particularly important for SDGs, which exhibit significant semantic overlap. For example, SDG 9 (Industry, Innovation and Infrastructure) shares vocabulary with SDG 11 (Sustainable Cities and Communities) (e.g. ``infrastructure,'' ``development,'' and ``sustainable''), making classification challenging for frequency-based models that cannot distinguish contextual meaning.
...
Our multi-label architecture (sigmoid activation per label, independent thresholding) allows the model to learn both synergies and distinctions between goals, unlike hierarchical approaches that predict goals sequentially. In practice, Vanderfeesten et al. (2021) have utilized a similar multi-label transformer approach to navigate these semantic overlaps in this specific SDG classification task.
...
As shown in Table 2, the TF-IDF baseline outperforms our BERT prototype at smaller data sizes, consistent with scaling law predictions that simpler models dominate in low-data regimes. At 4,000 training samples, BERT closes the gap and surpasses TF-IDF (0.717 vs 0.700 in Micro-F1), confirming the crossover has already occurred. BERT improves steadily with more data: 0.657 at 1k, 0.687 at 2k, and 0.717 at 4k, while TF-IDF shows diminishing returns: 0.611 at 1k, 0.672 at 2k, and 0.700 at 4k. Subset accuracy follows a similar pattern, with BERT 4k achieving 0.670 compared to TF-IDF 4k at 0.595.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\textwidth]{visualization/charts/fig_per_label_f1.png}
\caption{Per-SDG F1 scores for the BERT model at 4,000 training samples. Green $\geq$ 0.7, amber 0.5--0.7, red $<$ 0.5.}
\end{figure}
...
Figure 3 demonstrates how attention-weighted tokens help the human analyst: instead of reading all 128 tokens, they can glance at the ``top tokens'' like ``housing'' or ``urban'' for SDG 11 to quickly verify the AI's logic.
...
\caption{Figure 3: Example attention explanations showing top-attended tokens that help human analysts quickly verify predictions. Attention is a proxy explanation, not a validated rationale.}
\end{figure}
...
As shown in Figure 3, the attention tokens specifically help the human analyst: instead of reading entire documents, they can glance at the top-attended tokens (e.g., ``housing,'' ``urban'' for SDG 11) to quickly verify the AI's reasoning. This transforms the AI from a ``Black Box'' to a ``Grey Box'' that supports informed human decision-making.

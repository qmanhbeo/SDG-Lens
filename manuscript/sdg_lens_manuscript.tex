...
The UN's mission relies on accurate SDG tracking for resource allocation. If our model misses an SDG 5 (Gender Equality) label in a country's report, that country may receive less funding for women's programs. The stakes are not abstract, and they affect real funding decisions. We recommend that all automated SDG classifications be reviewed by human analysts.
...
\begin{itemize}
\item \textbf{UN Analyst Workflow (Human-AI Collaboration):} An analyst reviewing 50 VNR reports uses SDG Lens as a pre-screener with threshold 0.3 to get all possible SDG 7 documents (high recall), then filters false positives by checking attention tokens (``energy,'' ``renewable'') in the output.
\item \textbf{NGO Verification:} An NGO compares the model's predictions against a country's official project database to flag discrepancies for investigation.
\end{itemize}
...
\item Significant per-label variance (SDG 11: 0.764 vs SDG 5: 0.637) requires targeted improvements; future work should explore class-balancing techniques, compare with larger encoders (BERT-base, Deberta), and extend to multi-language VNRs (Spanish, French)

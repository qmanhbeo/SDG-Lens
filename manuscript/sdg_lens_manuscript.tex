...
Multi-label classification with 17 labels creates a sparse problem: on average only 1.44 labels are positive per document out of 17. This means a standard 0.5 threshold would predict zero labels for many documents. We select 0.3 as a recall-oriented threshold: it produces an average of 2.65 predicted labels per document (roughly double ground truth), so that the AI acts as a discovery tool finding all possible goals for a human analyst to filter. This directly addresses the expectation set in Section~\ref{sec:multilabel} that a low threshold is essential to avoid predicting zero labels.

\begin{table}[htbp]
\centering
\caption{Threshold sweep across 30 artifacts (31,710 test samples). Asterisk marks selected threshold.}
...
\begin{itemize}
\item \textbf{Best performers:} SDG 11 (Sustainable Cities, F1=0.778), SDG 9 (Industry, Innovation, F1=0.757), SDG 13 (Climate Action, F1=0.749), SDG 4 (Quality Education, F1=0.737)
\item \textbf{Weakest performers:} SDG 5 (Gender Equality, F1=0.636), SDG 7 (Clean Energy, F1=0.644), SDG 1 (No Poverty, F1=0.645)
\end{itemize}
...
Furthermore, aligned with our expectation in Section~\ref{sec:contextual}, SDGs with heavy semantic overlap (SDG 6, 9, 11, 14) are exactly where BERT outperforms TF-IDF most. BERT leads TF-IDF by 0.057 on SDG 9, 0.053 on SDG 6, 0.022 on SDG 11, and 0.008 on SDG 14\u2014whereas on less-overlapping labels like SDG 5 and SDG 7, the gap narrows.
...
\item \textbf{Example 2 (SDG 6, 11, 13, 17):} This text is from a flood mitigation project proposal. The model predicts SDG 6 (Clean Water), SDG 11 (Sustainable Cities), SDG 13 (Climate Action), and SDG 17 (Partnerships). Top tokens include "floods", "flood", "flooding", and "plan". The token "floods" and "flooding" are appropriate for water management and climate action, while "plan" aligns with the institutional language expected in SDG 17 partnerships. Notably, BERT picks up "flood" even in abbreviations like "##g" and "##an", suggesting it attends to partial word stems.
...
\item \textbf{Example 5 (SDG 11):} This text is from a section on cities and human settlements. The model predicts SDG 11 (Sustainable Cities). Top tokens include "settlements", "cities", "safe", and "communities". These are the defining terms of SDG 11, and BERT correctly highlights all of them. The token "##ili" is a partial token fragment from "inclusivity", which BERT partially decodes even in subword form.
\end{itemize}
...
The average predicted labels per document (1.53) is slightly higher than ground truth (1.44), suggesting the model tends to over-predict. Our threshold strategy (0.3 for recall) is deliberately designed for this: let the AI discover all possible goals, then let humans filter false positives.
...
\item Significant per-label variance (SDG 11: 0.778 vs SDG 5: 0.636) requires targeted improvements
...
\item Explore class-balancing techniques for underperforming labels (especially SDG 5 at 0.636 and SDG 7 at 0.644)

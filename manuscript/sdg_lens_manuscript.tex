...
Concretely, we take the final transformer layer's attention from the \texttt{[CLS]} token to each input token, average it across attention heads, mask padding and special tokens, and sort the remaining tokens by score. Therefore, ``top attended'' means highest average \texttt{[CLS]}-attention tokens, not words proven to cause the prediction.
...
This appendix describes the repository workflow exactly as implemented. The marker-facing command is \texttt{python main.py sweep}. It runs five stages in order: BERT training, TF-IDF baseline training, artifact evaluation, visualization generation, and LaTeX compilation. By default, the sweep uses training sizes \texttt{1000}, \texttt{2000}, and \texttt{4000}; seeds \texttt{42}, \texttt{43}, \texttt{44}, \texttt{45}, and \texttt{46}; language \texttt{en}; test seed \texttt{43}; threshold \texttt{0.3}; and device \texttt{cuda}. If all artifact metadata and referenced model files already exist, the training stages skip retraining unless \texttt{--force} is passed.

\subsection{Data Source and Files}

The local data files are copied SDGi Corpus parquet files stored as \texttt{data/train-00000-of-00001.parquet} and \texttt{data/test-00000-of-00001.parquet}. The dataset is the UNDP SDG Integration Corpus (SDGi Corpus), described in the repository \url{https://github.com/UNDP-Data/dsc-sdgi-corpus} and paper \url{https://ceur-ws.org/Vol-3764/paper3.pdf}. The source texts come from Voluntary National Reviews (\url{https://hlpf.un.org/vnrs}) and Voluntary Local Reviews (\url{https://sdgs.un.org/topics/voluntary-local-reviews}); the local dataset card lists license \texttt{CC BY-NC-SA 4.0}. The raw split contains \texttt{5880} train rows and \texttt{1470} test rows. After filtering to English, the current repository has \texttt{4225} train rows and \texttt{1057} test rows. Each row contains \texttt{text}, \texttt{embedding}, \texttt{labels}, and \texttt{metadata}; the model pipeline uses \texttt{text}, \texttt{labels}, and \texttt{metadata}. Labels are 1-based SDG identifiers from \texttt{1} to \texttt{17}.

\subsection{Sampling and Preprocessing}

Both model families filter by \texttt{metadata["language"] == "en"} and use the same coverage-aware sampling rule. If the requested sample size is less than the filtered split, the sampler first selects at least one available row for each SDG using \texttt{random.Random(seed)}, fills the remaining slots from a seeded shuffle of all other rows, truncates to the requested size, and shuffles the selected rows again. If the requested size is non-positive or at least the split size, it uses the full split after \texttt{pandas.sample(frac=1.0, random\_state=seed)}. For the test set, the sweep passes \texttt{test\_seed=43}; because \texttt{1470} exceeds the English test size, all \texttt{1057} English test rows are used in shuffled order.

TF-IDF text preprocessing replaces standalone numbers with \texttt{NUM}, removes punctuation, collapses whitespace, lowercases, and strips leading/trailing whitespace. BERT text preprocessing only converts missing text to an empty string, replaces the Unicode non-character \texttt{U+FFFE} with a space, collapses whitespace, and strips.

\subsection{BERT Training}

The BERT-family model is \texttt{sentence-transformers/all-MiniLM-L6-v2}. The tokenizer is loaded with \texttt{use\_fast=True}; by default the code uses local HuggingFace files only unless \texttt{--allow-download} is passed. Tokenization uses truncation, dynamic padding, \texttt{return\_tensors="pt"}, and \texttt{max\_length=128}. The model loads \texttt{AutoConfig} with \texttt{output\_attentions=True} and eager attention where supported, then loads \texttt{AutoModel}. The encoder is frozen except for the final \texttt{2} transformer layers by default; passing \texttt{--unfreeze-encoder} trains all encoder layers. The classifier pools the final hidden state of the \texttt{[CLS]} token, applies dropout using the encoder hidden dropout probability or \texttt{0.1}, and maps the hidden size to \texttt{17} logits with one linear layer. The loss is \texttt{BCEWithLogitsLoss}.

For each BERT run, the script seeds Python, NumPy, and PyTorch with the run seed. Training uses \texttt{AdamW}, learning rate \texttt{5e-5}, batch size \texttt{8}, \texttt{3} epochs, no scheduler, no class weighting, and gradient clipping at norm \texttt{1.0}. The training loader shuffles batches; the test loader does not. Probabilities are \texttt{sigmoid(logits)}. Predictions use \texttt{probability > 0.3}; if a row has no positive labels after thresholding, the highest-probability SDG is forced on as a top-1 fallback. Each run saves a checkpoint \texttt{model.pt}, \texttt{run\_config.json}, \texttt{metrics.json}, \texttt{examples.json}, \texttt{results.json}, and an \texttt{artifact.json} handoff file under \texttt{artifacts/bert\_train\{size\}\_seed\{seed\}/}.

\subsection{TF-IDF Baseline}

The baseline converts labels with \texttt{MultiLabelBinarizer(classes=range(1,18))}. It uses \texttt{TfidfVectorizer(strip\_accents="ascii", lowercase=False, ngram\_range=(1,1), min\_df=10, max\_df=0.9, max\_features=10000, binary=True)}. It trains one \texttt{SGDClassifier} per SDG label with \texttt{loss="hinge"}, \texttt{penalty="l2"}, \texttt{alpha=1e-4}, \texttt{shuffle=True}, \texttt{learning\_rate="optimal"}, \texttt{random\_state=seed}, \texttt{early\_stopping=True}, \texttt{validation\_fraction=0.2}, and \texttt{class\_weight="balanced"}. If a label column has only one class in a training slice, the saved model list stores \texttt{None} for that SDG. During baseline training metrics, such a column predicts the constant training value; during the separate evaluation stage, \texttt{None} columns predict \texttt{0}. Each baseline artifact saves \texttt{results.json}, \texttt{tfidf\_model.joblib}, and \texttt{artifact.json} under \texttt{artifacts/tfidf\_train\{size\}\_seed\{seed\}/}.

\subsection{Evaluation, Sweeps, and Reporting}

The evaluation stage discovers all \texttt{artifact.json} files under \texttt{artifacts/}. BERT artifacts are reloaded from \texttt{model.pt}, rebuilt with the checkpoint model name, threshold, batch size, max length, trainable-layer setting, and language, then evaluated on the same English test split. TF-IDF artifacts reload the saved vectorizer, per-label classifiers, and binarizer, then recompute predictions on the same English test split. Metrics are micro-F1, macro-F1, weighted-F1, subset accuracy, and per-label F1 with \texttt{zero\_division=0}. The summary groups rows by \texttt{model\_type} and \texttt{train\_size}, then reports means and sample standard deviations across available seeds.

The evaluation stage writes \texttt{results/evaluation\_by\_seed.csv}, \texttt{results/evaluation\_summary.csv}, \texttt{results/evaluation\_summary.json}, and \texttt{results/evaluation\_summary.md}. It also writes two threshold sweeps at thresholds \texttt{0.20}, \texttt{0.25}, \texttt{0.30}, \texttt{0.35}, \texttt{0.40}, \texttt{0.45}, and \texttt{0.50}: \texttt{threshold\_sweep.json} pools all evaluated artifacts, while \texttt{threshold\_sweep\_bert4k.json} pools only BERT artifacts with \texttt{train\_size=4000}. These sweeps use \texttt{probability >= threshold} and do not apply the BERT top-1 empty-prediction fallback. TF-IDF sweep scores are produced by applying a sigmoid transform to each classifier's \texttt{decision\_function} margin; these are diagnostic scores, not calibrated probabilities.

The visualization stage reads the evaluation CSV/JSON files. It writes summary tables to \texttt{outputs/tables/}, plots micro-F1 and macro-F1 bar charts, plots BERT-vs-TF-IDF per-label F1 at 4000 training samples, writes the manuscript threshold table from \texttt{threshold\_sweep\_bert4k.json}, and selects the largest seed-42 BERT result file for single-artifact metric, per-label, and explanation-example figures. Explanation figures show at most five examples. For each example, the report displays top label probabilities and the five highest-scoring non-special, non-padding attention tokens. Generated tables and charts are copied into \texttt{manuscript/visualization/}. The manuscript compiler checks that the required table and comparison charts exist, runs \texttt{pdflatex} twice with \texttt{-interaction=nonstopmode -halt-on-error}, writes the PDF, and removes temporary LaTeX files.
